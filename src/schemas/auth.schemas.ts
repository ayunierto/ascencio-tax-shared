...
export const signinSchema = z.object({
  ...
  password: z.string().min(8, { error: AuthMessages.PASSWORD_REQUIRED }),
});

export type SigninDto = z.infer<typeof signinSchema>;

export const signUpSchema = z.object({
  firstName: z.string().min(3, { error: AuthMessages.FIRST_NAME_REQUIRED }),
  lastName: z.string().min(3, { error: AuthMessages.LAST_NAME_REQUIRED }),
  email: z.email({ error: AuthMessages.INVALID_EMAIL }).nonempty({
    error: AuthMessages.EMAIL_REQUIRED,
  }),
  ...
  password: z.string().min(6, { error: AuthMessages.PASSWORD_REQUIRED }),
  confirmPassword: z.string().min(6, { error: AuthMessages.PASSWORD_REQUIRED }),
});
...
export const resetPasswordSchema = z.object({
  ...
  code: z.string().length(6, { error: CommonMessages.CODE_LENGTH }),
  newPassword: z.string().min(6, { error: AuthMessages.PASSWORD_REQUIRED }),
});

export type ResetPasswordDto = z.infer<typeof resetPasswordSchema>;

export const changePasswordSchema = z.object({
  currentPassword: z.string().min(6, { error: AuthMessages.PASSWORD_REQUIRED }),
  newPassword: z.string().min(6, { error: AuthMessages.PASSWORD_REQUIRED }),
});
